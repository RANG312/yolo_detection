...
def test_parse_args_loads_hikvision_env_file(monkeypatch, tmp_path: Path) -> None:
...
def test_resolve_images_uses_hikvision_capture_path(monkeypatch, tmp_path: Path) -> None:
...
def test_restore_hikvision_position_after_delay_waits_and_restores(monkeypatch) -> None:
    calls = []
...
def test_read_initial_hikvision_position_continues_when_restore_probe_fails(monkeypatch, capsys) -> None:
...
def test_start_callback_server_updates_callback_port_when_ephemeral_port_is_requested() -> None:
    ...
    try:
        ...
        assert (
            test_http.build_callback_url(
                Namespace(
                    callback_url=None, host="127.0.0.1", callback_port=args.callback_port, callback_path="/callback"
                )
            )
