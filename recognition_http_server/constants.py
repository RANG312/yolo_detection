...
DEFAULT_PTZ_ALIGN_MAX_DELTA_DEG = 2.0  # 单轮 pan/tilt 修正允许的最大角度
...
DEFAULT_PTZ_NUDGE_SPEED = 3  # 海康连续云台控制速度等级
DEFAULT_PTZ_NUDGE_DEGREES_PER_SECOND = 0.5  # 将目标角度换算为连续控制时长的估算速度，窄视场下取较低值
DEFAULT_PTZ_PAN_NUDGE_DEGREES_PER_SECOND = 12.0  # 当前云台 speed 档位下的水平实测角速度
DEFAULT_PTZ_TILT_NUDGE_DEGREES_PER_SECOND = 5.5  # 当前云台 speed 档位下的垂直实测角速度
...
DEFAULT_PTZ_ZOOM_TARGET_HEIGHT_RATIO = 0.5  # 目标表盘高度占整图高度的比例
DEFAULT_PTZ_ZOOM_RATIO_TOLERANCE = 0.05  # 表盘高度比例允许误差，误差内不调整 zoom
DEFAULT_PTZ_ZOOM_MAX_PASSES = 2  # 最多执行几轮“检测表盘 -> 调整 zoom -> 重新抓图”
DEFAULT_PTZ_ZOOM_NUDGE_SPEED = 2  # 海康 zoom 连续控制速度等级
